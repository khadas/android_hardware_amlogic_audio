...
int AudioFadeSetDirection(AudioFade_t *pAudFade, fade_direction_e eFadeDirection)
{
    ...
    pAudFade->mfadeDirection = eFadeDirection;

    if (pAudFade->mfadeDirection == FADE_AUD_IN) {
        pAudFade->mTargetVolume = 1 << 16;
        pAudFade->mStartVolume = 0;
        pAudFade->mCurrentVolume = 0;
    } else if (pAudFade->mfadeDirection == FADE_AUD_OUT) {
        pAudFade->mTargetVolume = 0;
        pAudFade->mStartVolume = 1 << 16;
        pAudFade->mCurrentVolume = 1 << 16;
    } else {
        ALOGE("%s,fade direction is %d", __FUNCTION__, eFadeDirection);
        return AUD_FADE_ERR;
    }

    ALOGI("%s,fade direction is %d", __FUNCTION__, eFadeDirection);
    return AUD_FADE_OK;
}
