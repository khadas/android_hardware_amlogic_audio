...
enum treblebass_state_e {
    TREBLEBASS_STATE_UNINITIALIZED,
    TREBLEBASS_STATE_INITIALIZED,
    TREBLEBASS_STATE_ACTIVE,
};

typedef enum {
    TREBLEBASS_PARAM_INVALID = -1,
    TREBLEBASS_PARAM_BASS_LEVEL,
...
typedef struct TREBLEBASSContext_s {
...
int TrebleBass_load_ini_file(TREBLEBASSContext *pContext)
{
    int result = -1;
    char ini_name[100] = {0};
    const char *ini_value = NULL;
...
int TrebleBass_init(TREBLEBASSContext *pContext)
{
    //TreBassdata *data = &pContext->gTreBassdata;
...
int TrebleBass_configure(TREBLEBASSContext *pContext, effect_config_t *pConfig)
{
...
int TrebleBass_getParameter(TREBLEBASSContext *pContext, void *pParam, size_t *pValueSize, void *pValue)
{
    ...
    case TREBLEBASS_PARAM_ENABLE:
        ...
        *(int32_t *) pValue = value;
        ALOGD("%s: Get status -> %s", __FUNCTION__, TREBLEBASSStatusstr[value]);
        break;
    default:
...
int TrebleBass_setParameter(TREBLEBASSContext *pContext, void *pParam, void *pValue)
{
    int32_t param = *(int32_t *)pParam;
    int32_t value;
    TreBassdata *data = &pContext->gTreBassdata;
    ...
    case TREBLEBASS_PARAM_ENABLE:
        ...
        ALOGD("%s: Set status -> %s", __FUNCTION__, TREBLEBASSStatusstr[value]);
        break;
    default:
...
int TrebleBass_release(TREBLEBASSContext *pContext __unused)
{
    return 0;
}
...
{
    TREBLEBASSContext * pContext = (TREBLEBASSContext *)self;
...
int dump(TREBLEBASSContext *pContext __unused)
{
    return 0;
}
        ...
        void *pCmdData, uint32_t *replySize, void *pReplyData)
{
    ...
    if (pContext == NULL || pContext->state == TREBLEBASS_STATE_UNINITIALIZED) {
        return -EINVAL;
    }
    ...
    case EFFECT_CMD_ENABLE:
        if (pReplyData == NULL || replySize == NULL || *replySize != sizeof(int))
            return -EINVAL;
        if (pContext->state != TREBLEBASS_STATE_INITIALIZED)
            return -ENOSYS;
        pContext->state = TREBLEBASS_STATE_ACTIVE;
        *(int *)pReplyData = 0;
        break;
    case EFFECT_CMD_DISABLE:
        if (pReplyData == NULL || replySize == NULL || *replySize != sizeof(int))
            return -EINVAL;
        if (pContext->state != TREBLEBASS_STATE_ACTIVE)
            return -ENOSYS;
        pContext->state = TREBLEBASS_STATE_INITIALIZED;
        *(int *)pReplyData = 0;
        break;
...
int TrebleBassLib_Create(const effect_uuid_t * uuid, int32_t sessionId __unused, int32_t ioId __unused, effect_handle_t * pHandle)
{
    ...
    TREBLEBASSContext *pContext = new TREBLEBASSContext;
    if (!pContext) {
        ALOGE("%s: alloc HPEQContext failed", __FUNCTION__);
        return -EINVAL;
    }
    memset(pContext, 0, sizeof(TREBLEBASSContext));
    ...
    pContext->state = TREBLEBASS_STATE_UNINITIALIZED;
...
int TrebleBassLib_Create_3_1(const effect_uuid_t * uuid, int32_t sessionId, int32_t ioId, int32_t deviceId __unused, effect_handle_t * pHandle)
{
    return TrebleBassLib_Create(uuid, sessionId, ioId, pHandle);
}

int TrebleBassLib_Release(effect_handle_t handle)
{
    TREBLEBASSContext * pContext = (TREBLEBASSContext *)handle;
...
audio_effect_library_t AUDIO_EFFECT_LIBRARY_INFO_SYM = {
    ...
    .create_effect_3_1 = TrebleBassLib_Create_3_1,
