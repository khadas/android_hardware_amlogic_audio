...
void DPFrequency::configure(size_t blockSize, size_t overlapSize __unused,
        size_t samplingRate) {
    //ALOGV("configure");
    mBlockSize = blockSize;
...
size_t DPFrequency::processSamples(const float *in, float *out, size_t samples) {
       ...
       mBlockSize = samples / 2;

       //ALOGW("mBlockSize: %zu", mBlockSize);
       //ALOGW("samples: %zu",samples);

       configure(mBlockSize, mOverlapSize, mSamplingRate);
...
size_t DPFrequency::processChannelBuffers(CBufferVector &channelBuffers) {
    ...
    //size_t processFrames = mBlockSize / 2;
