 ...
 */
#define LOG_TAG "audio_hw_dtvutils_string16"
// #define LOG_NDEBUG 0
...
dp_state String16::setTo(const String16& other, size_t len, size_t begin)
{
...
dp_state String16::setTo(const char16_t* other)
{
    return setTo(other, strlen16(other));
}

dp_state String16::setTo(const char16_t* other, size_t len)
{
...
dp_state String16::append(const String16& other)
{
...
dp_state String16::append(const char16_t* chrs, size_t otherLen)
{
    const size_t myLen = size();
    if (myLen == 0) {
        setTo(chrs, otherLen);
...
dp_state String16::insert(size_t pos, const char16_t* chrs)
{
    return insert(pos, chrs, strlen16(chrs));
}

dp_state String16::insert(size_t pos, const char16_t* chrs, size_t len)
{
    const size_t myLen = size();
    if (myLen == 0) {
...
dp_state String16::replaceAll(char16_t replaceThis, char16_t withThis)
{
    const size_t N = size();
    const char16_t* str = string();
    char16_t* edit = NULL;
...
dp_state String16::remove(size_t len, size_t begin)
{
    const size_t N = size();
    if (begin >= N) {
        SharedBuffer::bufferFromData(mString)->release();
