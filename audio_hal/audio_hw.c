                                    ...
                                    struct audio_stream_out *stream)
{
    ...
    if (adev && adev->audio_patching && (adev->patch_src == SRC_DTV || adev->patch_src == SRC_ATV) &&
        !adev->continuous_audio_mode && out->total_write_size != 0) {
    ...
    /*
     * will close MS12 if the AVR DDP-ATMOS capability is changed,
...
static int aml_audio_update_arc_status(struct aml_audio_device *adev, bool enable)
{
     ...
     * use "arc_hdmi_updated" parameter to notify HDMI ARC have updated status
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    //  HDMI plug in and UI [Sound Output Device] set to "ARC" will receive HDMI ARC Switch = 1
    //  HDMI plug off and UI [Sound Output Device] set to "speaker" will receive HDMI ARC Switch = 0
    ...
    */
    ...
    if (is_usecase_mix (aml_out->usecase) ) {
        ...
        if (usecase <= 1) {
            if (eDolbyMS12Lib == adev->dolby_lib_type) {
                if (!continuous_mode(adev)) {
                    // plug in HDMI ARC case, get_dolby_ms12_cleanup() will block HDMI ARC info send to audio hw
            ...
            for (int dev = AML_AUDIO_OUT_DEV_TYPE_SPEAKER; dev < AML_AUDIO_OUT_DEV_TYPE_BUTT; dev++) {
                ...
                if (dev == AML_AUDIO_OUT_DEV_TYPE_SPEAKER || dev == AML_AUDIO_OUT_DEV_TYPE_HEADPHONE) {
                    ...
                    if (aml_getprop_bool("vendor.media.audiohal.outdump")) {
                        if (dev == AML_AUDIO_OUT_DEV_TYPE_SPEAKER && adev->active_outport == OUTPORT_SPEAKER) {
                            aml_audio_dump_audio_bitstreams("/data/vendor/audiohal/audio_spk.pcm", adev->out_16_buf, bytes);
                        } else if (dev == AML_AUDIO_OUT_DEV_TYPE_HEADPHONE && adev->active_outport == OUTPORT_HEADPHONE) {
        ...
        if (!is_bypass_dolbyms12(stream) && (reset_decoder == true)) {
            ...
            if (adev->ms12.dolby_ms12_enable) {
                ...
                if (!audio_is_linear_pcm(aml_out->hal_internal_format)
                    ...
                    **handle the volume.
                    **So AudioHal should pass the audio data with volume full scale.
            ...
            ALOGI("%s() get_the_dolby_ms12_prepared %s, ott_enable = %d, main1_dummy = %d", __FUNCTION__, (ret == 0) ? "success" : "fail", ott_input, main1_dummy);
            pthread_mutex_unlock(&adev->lock);
...
hwsync_rewrite:
    ...
    if ((eDolbyMS12Lib == adev->dolby_lib_type) && !is_bypass_dolbyms12(stream) && !is_dts_format(aml_out->hal_internal_format)) {
        ...
        */
        ...
        /*during netflix pause, it will first pause, then ms12 will do fade out
...
static int adev_dump(const audio_hw_device_t *device, int fd)
{
    ...
    dprintf(fd, "\n");
    dprintf(fd, "[AML_HAL]      digital_audio_format     : %10d |  active_outport    :    %s\n",
        aml_dev->digital_audio_format, outputPort2Str(aml_dev->active_outport));
...
static int adev_set_audio_port_config(struct audio_hw_device *dev, const struct audio_port_config *config)
{
    ...
    /* find the corresponding sink for this src */
    list_for_each(node, &aml_dev->patch_list) {
        patch_set = node_to_item(node, struct audio_patch_set, list);
        patch = &patch_set->audio_patch;
