...
#define  DTV_AVSYNC_MS12_ARC_ATMOSOUT_LATENCY_PROP          "vendor.media.dtv.ms12.arc.atmosout.latencyms"
...
int aml_dtvsync_ms12v1_process_insert(void *priv_data, int insert_time_ms,
                                    aml_ms12_dec_info_t *ms12_info)
{
    ...
    if (is_mat)
        ...
        insert_ms = 32;
    ALOGI("%s, inset_time_ms=%d, insert_ms=%d, t1=%d, is_mat=%d\n", __func__, insert_time_ms, insert_ms, t1, is_mat);
    memset(patch->out_buf, 0, patch->out_buf_size);
    do {
        if (patch->output_thread_exit == 1) {
            ALOGI("input exit, break now\n");
...
dtvsync_process_res aml_dtvsync_ms12v1_process_policy(void *priv_data, aml_ms12_dec_info_t *ms12_info)
{
    struct aml_stream_out *aml_out = (struct aml_stream_out *)priv_data;
    ...
    if (aml_dtvsync != NULL) {
        ...
        ALOGI("audiopolicy:%d, prm1:%d, prm2:%d\n", async_policy->audiopolicy,
        ...
        if (async_policy->audiopolicy == MEDIASYNC_AUDIO_DROP_PCM) {
            ...
            aml_dtvsync_ms12v1_process_insert(priv_data, async_policy->param1/1000, ms12_info);
