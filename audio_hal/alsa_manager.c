...
#define ALSA_DUMP_PROPERTY       "media.audiohal.alsadump"
#define ALSA_OUTPUT_PCM_FILE     "/data/vendor/audiohal/alsa_pcm_write.pcm"
#define ALSA_OUTPUT_SPDIF_FILE   "/data/vendor/audiohal/alsa_spdif_write.pcm"
...
static int insert_eff_zero_bytes(struct aml_audio_device *adev, size_t size)
{
    ...
    while (insert_size > 0) {
        ...
        /*
         * do not need to do the aduio effect process for speaker
         * becasue it will produces a crash at VirtualX's Virtualx_process function
         */
        /*
    ...
    if (aml_out->pcm && pcm_is_ready(aml_out->pcm)) {
        if (pcm_ioctl(aml_out->pcm, SNDRV_PCM_IOCTL_DELAY, &frames) >= 0) {
            if (adev->sink_format == AUDIO_FORMAT_E_AC3
                || adev->sink_format == AUDIO_FORMAT_DTS_HD) {
                frames  = frames / 4;
            }
...
size_t aml_alsa_output_write_new(void *handle, const void *buffer, size_t bytes)
{
    ...
    if (getprop_bool(ALSA_DUMP_PROPERTY)) {
        aml_audio_dump_audio_bitstreams(ALSA_OUTPUT_SPDIF_FILE, buffer, bytes);
    }
